	...
	resp.Schema = schema.Schema{
		Attributes: map[string]schema.Attribute{
			"host": schema.StringAttribute{
				Required:    true,
				Description: "A fully qualified hostname (e.g. https://myapi.structurizr.com)",
			},
			...
			"tls_insecure": schema.BoolAttribute{
				Optional:    true,
				Description: "Disable TLS verification checks for privately hosted structurizr",
			},
		},
	}
}
...
func (p *Structurizr) Configure(ctx context.Context, req provider.ConfigureRequest, resp *provider.ConfigureResponse) {
	...
	c := client.NewStructurizr(client.Config{
		AdminAPIKey:     adminApiKey,
		BaseURL:         baseURL,
		CustomTLSConfig: &tls.Config{InsecureSkipVerify: tlsInsecure},
